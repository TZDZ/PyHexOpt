...
from pyhexopt.core.neighbor import build_element_adjacency, get_element_neighborhood
...
def test_build_element_adjacency(simple_mesh):
    adj = build_element_adjacency(simple_mesh)

    # adjacency is square and boolean
    E = simple_mesh.shape[0]
...
def test_first_neighborhood(simple_mesh):
    nodes, elems = get_element_neighborhood(simple_mesh, elem_index=0, depth=1)
    ...
    used_nodes = jnp.unique(simple_mesh[elems].ravel())
    assert jnp.array_equal(jnp.sort(nodes), used_nodes)


def test_second_neighborhood(simple_mesh):
    nodes, elems = get_element_neighborhood(simple_mesh, elem_index=0, depth=2)
...
def test_isolation_case():
    ...
    nodes, elems = get_element_neighborhood(cells, elem_index=0, depth=3)
...
def test_8x8x8():
    ...
    nodes, elems = get_element_neighborhood(cells, elem_index=456 - 1, depth=1)
    assert len(elems) == 8
    nodes, elems = get_element_neighborhood(cells, elem_index=8 - 1, depth=1)
    assert len(elems) == 8
    nodes, elems = get_element_neighborhood(cells, elem_index=7 - 1, depth=1)
    assert len(elems) == 12
    nodes, elems = get_element_neighborhood(cells, elem_index=8 - 1, depth=2)
    assert len(elems) == 3**3
    nodes, elems = get_element_neighborhood(cells, elem_index=8 - 1, depth=3)
